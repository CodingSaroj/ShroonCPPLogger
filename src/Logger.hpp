...
// Debug mode macros
    ...
    // If log level is set above or at Error then define PG_ERROR and PG_ASSERT.
        ...
        #define PG_ASSERT(cond, section, ...) if (!(cond)) { Peregrine::Logger::LogFatalError(section, Format(__VA_ARGS__)); }
    #else
        #define PG_ERROR(...)
...
#if PG_LOG_LEVEL >= PG_LOG_LEVEL_WARNING
    #define PG_REL_WARNING(section, ...) PeregrineLogger::LogWarning(section, Format(__VA_ARGS__))
#else
...
// If log level is set to Error then define PG_REL_ERROR and PG_REL_ASSERT.
    ...
    #define PG_REL_ASSERT(cond, section, ...) if (!(cond)) { Peregrine::Logger::LogFatalError(section, Format(__VA_ARGS__)); }
#else
    #define PG_REL_ERROR(...)
